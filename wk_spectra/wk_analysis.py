...
class wk_analysis(object):
    ...
    def import_netcdf(self,file,varname,latname='latitude',latBound=20):
    ...
    def wheeler_kiladis_spectra(self,spd,nDayWin,nDaySkip,max_freq=0.8,max_wn=20):
        if (self.data_status=="Empty"):
            ...
            if (self.analysis_status=="Empty"):
                ...
                psumanti = xr.DataArray(name='psumanti', data=psumanti,
                                       dims=['frequency','wavenumber'],
                                       coords={'frequency':freqfft,
                                               'wavenumber':wavefft,
                                              })
                psumsym  = xr.DataArray(name='psumsym', data=psumsym,
                                       dims=['frequency','wavenumber'],
                                       coords={'frequency':freqfft,
                                               'wavenumber':wavefft,
                                              })
                psumb   = xr.DataArray(name='psumb', data=psumb,
                                       dims=['frequency','wavenumber'],
                                       coords={'frequency':freqfft,
                                               'wavenumber':wavefft,
                                              })
                psumanti_r = xr.DataArray(name='psumanti_r', data=psumanti_r,
                                       dims=['frequency','wavenumber'],
                                       coords={'frequency':freqfft,
                                               'wavenumber':wavefft,
                                              })
                psumsym_r = xr.DataArray(name='psumsym_r', data=psumsym_r,
                                       dims=['frequency','wavenumber'],
                                       coords={'frequency':freqfft,
                                               'wavenumber':wavefft,
                                              })
                ...
                self.wk_spectra_xr = xr.merge([psumanti, psumsym, psumb, psumanti_r, psumsym_r])
                self.wk_spectra = {
                            'psumanti':psumanti,
                            'psumsym':psumsym,
                            'psumb':psumb,
    ...
    def plot_dual_background_removed(self,figsize=(18,8),text_size=12,contour_range=(0,2,0.1),min_contour_range_lines=1.1,\
            ...
            for ax in axAS:
                ...
                ax.set_xlabel('Zonal Wavenumber',size=text_size,fontweight='bold')
                ax.set_ylabel('Frequency (CPD)',size=text_size,fontweight='bold')
                ax.text(self.max_wn_plot-2*0.25*self.max_wn_plot,-0.08,'EASTWARD',fontweight='bold',fontsize=text_size-2)
...
#         np.save(name,self.wk_spectra)
        name = name+'.wk_spectra.nc'
        self.wk_spectra_xr.to_netcdf(name)
